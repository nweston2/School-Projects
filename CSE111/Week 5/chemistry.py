...
Author: Noah Weston
Date: 26 Nov 2022
Assignment: 08 Prove Assignment: Dictionaries
Purpose: Assemble a chemistry database
"""
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    """
    nwTotalMass = 0
    # Do the following for each each inner list in the compound symbol_quantity_list:
    for element in symbol_quantity_list:
        # Separate the inner list into symbol and quantity.
        nwSymbol = element[0]
        nwQuantity = element[1]

        # Get the atomic mass for the symbol from the dictionary.
        nwElementInfo = periodic_table_dict[nwSymbol]
        nwAtomicMass = nwElementInfo[1]

        # Multiply the atomic mass by the quantity.
        nwElementMass = nwQuantity * nwAtomicMass

        # Add the product into the total molar mass.
        nwTotalMass += nwElementMass
...
def main():
    ...
    nwFormula = input("Enter the chemical formula: ")

    # Get the mass of a chemical sample in grams from the user.
    nwAmount = float(input("Enter mass in grams: "))

    # Call the parse_formula function to convert the
    # chemical formula given by the user to a compound
    # list that stores element symbols and the quantity
    # of atoms of each element in the molecule.
    nwList = parse_formula(nwFormula, periodic_table_dict)

    # Call the compute_molar_mass function to compute the
    # molar mass of the molecule from the compound list.
    nwMolarMass = compute_molar_mass(nwList, periodic_table_dict)

    # Compute the number of moles in the sample.
    nwMoles = nwAmount / nwMolarMass

    # Print the molar mass.
    print(f"{nwMolarMass:.5f} grams/mole")

    # Print the number of moles.
    print(f"{nwMoles:.5f} moles")
